--- src/flow-print.c.orig	2009-02-10 11:30:09.000000000 +0200
+++ src/flow-print.c	2009-12-04 14:36:46.000000000 +0200
@@ -77,15 +77,16 @@
...
+
+  puts("Start             End               P Sif   SrcIPaddress    SrcP  DIf   DstIPaddress    DstP    Pkts       Octets Fl ToS SrcAS DstAs\n");
+
...
+        
+    printf("%-4u %-5u %-15.15s %-5u <-> %-5u %-15.15s %-5u %-10lu %-10lu %1s%1s%1s%1s%1s%1s %-2.2x %-8.8s %-8.8s\n",
+
...
+           (unsigned int)*cur.tos, fmt_buf3, fmt_buf4);
+
